"""
    ...
    Functions/classes required for bit operations.
"""
...
class Bits:
    """Class provides methods for bit operations."""
    ...
    def getbitu(cls, buf: bytes, start_pos: int, length: int) -> int:
        ...
        if finish_byte > len(buf):
            raise ExceptionBitsError(
                f"getbitu overshoot:{finish_byte=},len={len(buf)}"
            )
