...
class EphemerisDecoder(Bits):
    ...
    def __length_check(Nexp: int, Ndec: int, bufLen: int):
        ...
        if Ndec != Nexp:
            raise ExceptionEphemerisDecoder(
    ...
    def __decode1019(self, buf: bytes) -> mEph.EphGPS:
        ...
        eph.L2P_Data, offset = self.getbitu(buf, offset, 1), offset + 1  # DF103
    ...
    def __scale1019(cls, ie: mEph.EphGPS, decorate: bool = False) -> mEph.EphGPS:
        ...
        if decorate:
            eph.L2_Codes = _L2_Codes[ie.L2_Codes]  # DF078 # type: ignore
            eph.URA = ura(ie.URA)  # DF077 # type: ignore
    ...
    def scale(cls, ie: object) -> object:
        ...
        match type(ie):
            case mEph.EphGPS:
                return cls.__scale1019(ie)  # type: ignore
            case mEph.EphGLO:
                return cls.__scale1020(ie)  # type: ignore
            case mEph.EphGALF:
                return cls.__scale1045(ie)  # type: ignore
            case mEph.EphGALI:
                return cls.__scale1046(ie)  # type: ignore
            case mEph.EphBDS:
                return cls.__scale1042(ie)  # type: ignore
            case mEph.EphNAVIC:
                return cls.__scale1041(ie)  # type: ignore
            case mEph.EphQZS:
                return cls.__scale1044(ie)  # type: ignore
            case _:
                raise ExceptionEphemerisDecoder(
                    f"scaler doesn't support type {type(ie)}"
                )
...
class SubdecoderEph:
    ...
    def decode(self, buf: bytes) -> Any | None:
        """Find decoder and convert bytes to data object."""
